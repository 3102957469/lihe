...
public class WorkflowController {
    ...
    public Object start(@RequestBody InputVO inputVO) {
        ProcessInstance it = runtimeService.startProcessInstanceByKey(inputVO.getProcessKey(), inputVO.getVariable());  //画流程图时设置的process_id
        Map map = new HashMap();
        map.put("getProcessInstanceId", it.getProcessInstanceId());
        map.put("getBusinessKey", it.getBusinessKey());
        map.put("getProcessDefinitionId", it.getProcessDefinitionId());
        map.put("getId", it.getId());
        return map;
    }
    ...
    @RequestMapping("job")
    public void job(@RequestBody InputVO inputVO) {
        managementService.executeJob(inputVO.getTaskId());
    }

    /**
     *触发信号中间事件
     */
    @RequestMapping("signalEvent")
    public Object signalEvent(@RequestBody InputVO inputVO){
        List<Execution> list = runtimeService.createExecutionQuery()
                .processInstanceId(inputVO.getProcessId())
                .signalEventSubscriptionName(inputVO.getNextSignal())
                .list();
    ...
    @RequestMapping("createJobQuery")
    public Object createJobQuery(@RequestBody InputVO inputVO) {
